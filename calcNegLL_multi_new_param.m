function [LL, dLL] = calcNegLL_multi_new_param(Xs, Y, U, Ps, Pcs, Xo, Po, Theta)
...
[alpha beta Vrev Ke Q R] = deal(Theta(1), Theta(2), Theta(3), ...
...
%% calc LL
temp=(Xs(2:end)-alpha*Xs(1:end-1)-beta*KeU(1:end-1));
L11 = temp*temp' + P11 -2*alpha*P10 + alpha^2*P00;
L1 = L11 / Q;
L2 = (N-1)*log(abs(Q));

L33 = (Y-Vrev-Xs-KeU)*(Y-Vrev-Xs-KeU)' + P;
L3 = L33/R;
L4 = N*log(abs(R));
...
% %     SYU = Y*U';
    ...
    A1 = [SS10+P10 Xs(:,2:end)*KeU(1:end-1)'];
    A2 = [SS00+P00 XKeU0; XKeU0 KeU(1:end-1)*KeU(1:end-1)'];
    dLdAB = 2*Qi*( A1 - [alpha beta]*A2);
    
    
    %% calculate new Ke
    %Ke = (SYU-C*SXU)/SUU;      % simple approximation
    K1 = (beta*Qi*(SXU10-alpha*SXU00-gamma*SU0')+Ri*((Y-Vrev)*U'-C*SXU));
