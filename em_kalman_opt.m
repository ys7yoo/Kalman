function [paramEstim LLs Xs] = em_kalman_opt(Y, U, paramInit, max_iter, const)
...
while (itr <=max_iter) %% && (~converged) % && (~decreased)
    ...
    if 1 %%( changeLL > eps )    % check LL is increasing 
        [alpha beta gamma Ke Q R Xo Po]=m_step_opt(alpha, beta, gamma, Ke, Q, R, Y, U, Xo, Po, Xs, Ps, Pcs, const);
