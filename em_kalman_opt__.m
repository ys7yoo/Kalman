function [paramEstim LLs Xs] = em_kalman_opt(Y, U, paramInit, max_iter)
...
while (itr <=max_iter) && (~converged) && (~decreased)
    ...
    if ( changeLL > eps )    % check LL is increasing 
        ...
        lossFunc=@(Theta) (calcNegLL (Xs, Y, U, Theta));
        ...
        Theta = fminunc(lossFunc,Theta);
% %         lb = [0 -Inf -Inf zeros(1,M)   0   0  -Inf 0];
% %         ub = [1 Inf Inf Inf*ones(1,M) Inf Inf Inf Inf];
% %         Theta = fmincon(lossFunc,Theta,[],[],[],[],lb,ub);
...
% %         Po=Ps(:,:,1);
        ...
        if ( changeLL < 1e-3) 
            disp(sprintf('LL converges to %e in itr %d', LL, itr))
            converged = 1;
        end
